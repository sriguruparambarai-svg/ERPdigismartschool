// DigiSmart ERP — Secure Salary Data API (Database Lockdown Phase 3, Group 1)
// The ONLY door to the five salary tables. Verifies the signed session
// token from login, allows only owners and staff with the HRM module,
...
module.exports = async (req, res) => {
  ...
  const hasHrm = Array.isArray(session.mods) && session.mods.indexOf('hrm') !== -1;
  if (!isOwner && !hasHrm) {
    return res.status(403).json({ ok: false, error: 'You do not have permission for salary data.' });
  }
