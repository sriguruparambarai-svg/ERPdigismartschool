// DigiSmart ERP — Secure Scheme Data API
// The ONLY door to the two 5-Year Scheme tables. Verifies the signed session
// token issued at login, then talks to the database with the service key.
// Browsers can never reach these tables directly.
//
...
module.exports = async (req, res) => {
  ...
  // Row limit applies to reads only — an insert must never be capped
  if (q.action === 'select') {
    let limit = null;
    if (q.single) limit = 1;
  ...
  try {
    ...
    console.error('scheme-data error:', err);
