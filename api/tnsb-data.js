// DigiSmart ERP — Secure TN Assessment Data API (added 22-07-2026)
// The ONLY door to the TN State Board assessment tables (FA marks and
// co-scholastic grades). Same pattern as hrm-data.js: verifies the signed
// session token, allows only owners and staff with the Exam module,
// and locks every query to the token holder's own school.
//
...
module.exports = async (req, res) => {
  ...
  const hasExam = Array.isArray(session.mods) && session.mods.indexOf('exam') !== -1;
  if (!isOwner && !hasExam) {
    return res.status(403).json({ ok: false, error: 'You do not have permission for assessment data.' });
  }
  ...
  try {
    ...
    console.error('hrm-data error:', err);
